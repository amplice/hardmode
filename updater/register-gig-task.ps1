param(
  [string]$TaskName = "Gig Radar Auto Update (Codex)",
  [int]$DaysInterval = 3,
  [string]$At = "07:30"
)
...
# main calendar task (08:00) so the two headless Codex runs do not overlap.
...
Register-ScheduledTask `
  ...
  -Principal $Principal `
  -Settings $Settings `
  -Description "Runs Codex CLI non-interactively to research and update the Gig Radar (traditional jazz + roots-Americana gigs), then publishes to GitHub Pages. Logs under updater\logs." `
  -Force

Write-Host "Registered scheduled task: $TaskName"
Write-Host "Runner: $Runner"
Write-Host "Schedule: every $DaysInterval days at $At (offset from the calendar task at 08:00)"
Write-Host "Mode: Codex exec + auto-publish"
