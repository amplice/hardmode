...
## 2026-09-19
- Date basis: 19 September 2026, Europe/London. Read the contract, strategy, preferences, exported feedback, sources, latest changes and recent method history before editing. Removed 12 dated rows whose effective end was before today; retained active multi-day runs and all 56 genuine recurring patterns.
- Source refresh: probed all 85 configured known-source URLs and ran all 97 saved search queries. Initial probes returned 70 HTTP 200 responses, eight 403s, three 429s, one 406, one 404 and two DNS failures. Successful endpoint responses establish availability, not event-date verification; candidates were checked on detail pages/search-indexed official content. Later Surbiton What's On and news fetches were blocked; the indexed all-category feed and current festival site supplied the fallback.
- Methods rotated beyond the recent Community Brain/Green Note/residents-archive pass:
  - VILLAGE SHOP AND PTA ORGANISER ROUTE: searched current Thames Ditton shop/fair organisers rather than archived magazines. Weston Green School PTA's CharityHive page confirms 28 November 10:00-14:30, but does not explicitly establish access for non-school families. Watchlisted. Bakery Christmas copy still refers to Sunday 1 December and was not converted into a 2026 event.
  - MUSEUM UNDER-5 DATE RECONCILIATION: compared visitor-directory Snappity dates with Kingston Heritage's own detail and family pages. The museum detail still ends 1 June 2026; another visitor result calls an October Sunday a Saturday. Watchlisted pending current ticket-session dates rather than inventing a weekly series.
  - GARDEN-CENTRE LINKTREE TO DIGITICKETS: followed Chessington Garden Centre's official links into the Halloween circus and Christmas products. They expose all-ages wording and prices but no readable dated sessions. Watchlisted; rejected stale February circus dates and did not treat VR rides as toddler programming.
- What worked:
  - Quacky Races now has a consistent official title, date table and all-ages terms for 19 September, 11:00-15:00, at Morden Hall Park. Added the corrected current date. The old 13 September event had been removed in a previous run; this is corrected organiser evidence, not a feedback exception.
  - Mandatory cross-calendar QA covered all 270 future Gig Radar rows, including priority acts, daytime/free sessions and weekend shows. Promoted 11: four 2027 TwickFolk singarounds (strong positive feedback match), four free January TJ Johnson afternoons, the 3 January Prospective Collective afternoon, Fidelio Hot Club's November matinee and the January Benny Goodman recreation. Evening TwickFolk rows are adult outings, not toddler-bedtime recommendations. Cadogan and Toulouse rows retain verification flags for young-child admission.
  - Jamboree's artist-date inventory revealed further missing Prospective Collective dates. Checked individual links separately; most returned cache misses/406. Official indexed detail independently confirms 18 October 15:00-17:00/free, so added that additional row. The 3 January date is confirmed in the artist diary, while time/free entry come from the 17 September Gig Radar snapshot, so verify remains true. Nine other dates remain grouped only as a watchlist lead, not as an event range.
  - Official Jamboree indexed detail/dated pagination confirmed each January TJ Johnson date (6, 13, 20, 27), 15:00-17:00 and free/donations. TwickFolk's diary and separate pages confirmed 3/31 January, 28 February and 4 April, 19:45 and GBP5. Each is a separate row. Extended the calendar range to 4 April 2027.
  - Repaired Surbiton Festival's known-source and held-event URLs to surbitonfestival.com after the organiser explicitly announced its domain move. Added Cadogan Hall as a focused jazz-matinee source and three searches: 86 sources and 100 queries now configured.
- What did not work / cautions:
  - Spice of Life's calendar returns a verification challenge/403. Five otherwise relevant lunchtime leads from Gig Radar remain watchlisted until current detail pages can be rechecked. Bookham/Winning Post dates still lack reliable years; U3A rows, weak far-field trips, older-child activities and feedback-rejected categories were not promoted.
  - Reviewed the fresh 108-lead social export, especially new posts since the last run. Lamb's newest capture is an undetailed performance reel; other Lamb leads were already held or past. New Ewan posts include a past substitute afternoon, a late birthday jam and an evening Turkish-classical show. No additional toddler-plausible dated social row cleared the threshold. Social post timestamps were never treated as event dates.
  - Wimbledon Village Stables now explicitly confirms 5 December 2026, but publishes only the ticket-release time (1 November 08:00), not concert time/price. Watchlisted. Lost in the Loop's dated GBP12 matinee is held as a lead pending admission/time confirmation. No negative-feedback exceptions were made.
- Validation and file boundary:
  - Ran npm run generate followed by npm run validate in a temporary repository-shaped copy because the generator unconditionally writes calendar-data.js, events.ics, manifest.webmanifest, sw.js and icons/icon.svg outside the unattended-run allowlist. Copied back only the enriched calendar-data.json. Publishing must regenerate runtime assets.
  - Both commands passed: 312 events/feed events, 56 recurring rows, 53 calendar source rows, 84 watchlist entries, zero past-finished rows, zero duplicate keys; latest end 4 April 2027. Separately asserted unique IDs and exact duplicate keys. No commit or push.
- Try next time:
  - Resolve the five Spice lunchtime leads and nine remaining Prospective Collective dates from readable per-event pages; do not assume all Sunday programmes match.
  - Inspect Chessington's live dated ticket sessions; confirm Weston Green fair public access and obtain current Snappity Eventbrite dates.
  - On 1 November recheck Wimbledon Village Stables concert times/prices. Keep museum/vendor stale-year results filtered and use precise source-date queries when broad Jamboree searches return older events.


## 2026-09-22
- Date basis: 22 September 2026, Europe/London. Read HANDOFF, strategy, preferences, exported feedback, source inventory, latest changes, recent method history and the fresh social export before editing. Removed 22 rows with effective end dates before today; retained all active runs and all 56 recurring records unchanged. No feedback exceptions.
- Source refresh: probed all 86 configured URLs and ran all 100 saved search queries. Successful probes indicate endpoint availability, not date verification. Surbiton Partnership returned empty HTTP 202 challenge pages; indexed all-category listings supplied the fallback. Other failures included 403, 429, 401, 406, the stale Sofar 404 and Visit Kingston DNS failure. National Theatre was also checked and returned 403. Existing Hampton Court, railway, Polka and Southbank offerings were largely already held; stale summer and older-child results were excluded.
- Methods rotated beyond the recent museum/shop/PTA/garden-centre passes:
  - ARTIST-DIARY BACKFILL FOR SOCIAL-HEAVY VENUES: Mambo Jambo's current September 2026 calendar independently confirms The Lamb on 24 September. The detail page has no performance time or admission terms; added with verify true and no invented time. Lamb's own stale Fiddly Diddly copy was ignored under explicit preferences.
  - WILDLIFE-TRUST AGE/AVAILABILITY FILTER: checked Surrey Wildlife Trust's official event inventory for early-years outdoor possibilities. The near-term family night walk is fully booked and late; holiday clubs are ages 5-11 or teens, and other entries are adult courses/wellbeing. No additions.
  - EMERGENCY-SERVICE OPEN-DAY SEARCH: searched Kingston/Surbiton fire-station public open days. The official New Malden result was 28 June 2026, already past; no future date was inferred from its fresh crawl timestamp.
  - ROSE CURRENT WEBSITE DETAIL RECONCILIATION: followed live What's On links to the venue's current CultureSuite detail pages, reading each explicit selected-date list plus structured year/time data. This found 15 missing rows: three Una Coello themed baby-to-5 singalongs, four Melbury Music Quintet dates, two Teresa Jennings lunches and six Jazz Fridays. Added separate one-day rows; did not interpret the umbrella date ranges as daily runs. Singalongs use the published 10:00 start and state the 45-minute performance duration.
- Mandatory cross-calendar QA: reviewed all 273 future Gig Radar rows against the family calendar, including daytime/free gigs, weekends and Ewan Bleach/Doolally Tap priority dates. Existing Ewan lunch dates were already held despite differing titles/URLs. Promoted seven missing rows: Mambo Jambo at The Lamb; 25 September ICC free family trad session; Matt Holborn's 30 September Cafe Boheme set; and four 2027 TwickFolk shows (Counter's Creek, The Kimberleys, George Sansome & Matt Quinn, Tarren). TwickFolk is a strong positive-feedback match and is described as an adult evening outing, with 19:45 doors rather than a guessed music start.
- Current ICC HTML confirms 25 September, 17:30-20:00, free and children welcome, while the search cache still displays 18 September. Used current official content. Cafe Boheme's September roster confirms 28 September TJ Johnson and 30 September Matt Holborn, both 16:00-18:30. Added TJ separately from discovery; both cafe rows retain verify true for child seating/admission/dining arrangements. No unsupported free-entry claim.
- Unresolved gig leads: Spice of Life still returns a verification challenge; updated its watchlist to remove the past 21 September lead and retain four future lunches. Added an Epsom hot-jazz admission/date/price watchlist lead rather than assuming toddler access or a direct Surbiton train. Distant Bookham/Southend outings, U3A/member events, late central residencies and out-of-region tours did not clear this family's inclusion threshold. Existing Doolally and Ewan social/uncertain rows retain their verification flags.
- Reviewed the 108-lead social export, focusing on changes since 19 September and all Lamb posts. New material was mainly retrospective, generic acoustic/modern jazz, blues-rock or late programming. No new social-only family row cleared the threshold. Post dates were not treated as event dates.
- Added a watchlist lead for a possible Celebrate Surbiton October extension: council listings show 1 and 8 October, but organiser copy still specifies September only. Existing September weekly block retained; no guessed extension.
- Source improvements: added Mambo Jambo's artist diary, Cafe Boheme's dated roster and ICC family sessions; added three targeted searches. Now 89 known sources and 103 saved queries.
- Validation and file boundary: combined inline mutation commands were rejected by tool policy before execution; smaller file-specific operations and explicit temporary Python scripts succeeded. Ran npm run generate and npm run validate in a temporary repository-shaped copy to keep generated JavaScript, ICS, icons, manifest and service worker outside the repository changes. Copied back only enriched calendar-data.json. Publishing must regenerate runtime assets.
- Final checks passed: 313 events/feed events, 56 recurring records, 53 calendar source rows, 86 watchlist entries; zero past-finished rows and zero duplicate keys; latest end 25 April 2027. Separately confirmed unique IDs/row numbers, all new rows above the original maximum, unchanged retained event objects and unchanged recurring records. Only the four permitted maintenance files changed. No commit or push.
- Try next time: use Rose's live detail links and selected-date lists rather than relying on its legacy Spektrix feed; recheck Lamb start/admission details and Cafe Boheme child seating; reconcile the possible October forecourt extension with the organiser. Resolve Spice/Epsom lunch access before promotion. Prefer current official HTML over cached weekly ICC snapshots.


## 2026-09-25
- Date basis: 25 September 2026, Europe/London. Read the contract, strategy, family preferences, private feedback, source inventory, current social export, latest changes and recent method history. Removed eight events whose effective end was before today; preserved active multi-day runs and all 56 recurring patterns. No feedback exceptions.
- Source refresh: fetched all 89 configured known-source URLs and ran all 103 saved search queries. Direct responses: 76 HTTP 200, seven 403, three 429, one 404, one 406 and one DNS failure. Successful responses establish availability, not event-date verification. National Theatre was checked separately and returned 403. Surbiton Partnership's full all-category feed and individual detail pages were readable this time; official indexed material supplied fallbacks for blocked sources.
- Methods rotated beyond the recent artist-diary/wildlife/open-day/Rose passes:
  - NATIONAL PARTICIPATORY-ARTS MAP: checked Fun Palaces' 2026 directory and focused Kingston/Surbiton/Wimbledon searches. The national weekend is confirmed, but no sufficiently documented local toddler event emerged; direct directory retrieval returned 403. No generic national date was turned into a local event.
  - PUBLIC FARM ACTIVITY TO BOOKING-SESSION CHECK: examined Deen City Farm's current seasonal inventory and followed separate wildlife-painting, mask and Halloween-party BookingHound links. Current copy has promising family activities but omits event years; ticket detail fetches failed. Watchlisted for explicit year/age checks, avoiding the separate stale 2025 Spooktober article and school-age garden activities.
  - SPORTS-MUSEUM COMMUNITY TICKETING: Wimbledon Museum's booking inventory exposes six exact October-December under-5 sessions at 10:30, including Raynes Park editions. The page addresses Merton/Wandsworth residents. Watchlisted pending Surbiton eligibility rather than adding a potentially restricted activity.
  - INDUSTRIAL-HERITAGE PUBLIC/EDUCATION SPLIT: checked London Museum of Water and Steam's live programme and early-years offering. The latter is a school-group booking, while the public steam/train programme lacks clear years in readable copy and has a contradictory December weekday label. No speculative or school-only addition.
- What worked:
  - Surbiton detail-page sweep found five missing doorstep rows: Celebrate Surbiton at Christmas (3 December, 16:30-20:00, free, reindeer trail/music/stalls); public Legion carols (29 November, 17:00-19:00, free); Berrylands Belles benefit concert (11 October); and separate Elm Tree jazz jams (5 October and 2 November). Wilderness Yet, Jez Lowe and Bach to Baby were already held despite alternate listing URLs, so not duplicated. Evening music is explicitly described as an adult outing.
  - Positive Maiastra feedback raised its current autumn/winter programme. Checked October, anniversary and December pages separately. Added Twickenham's 3 October piano quartets and Cobham's 13 December afternoon string quartets, with confirmed programmes, free admission/donations and booking guidance. More awkward evening destinations were not expanded indiscriminately.
  - Mandatory cross-calendar QA covered all 274 future Gig Radar rows, comparing URLs plus date/venue/artist to catch title variants. Promoted the 1 November Riverhouse Barn Jazz & Beer concert: official /events/172497 JSON-LD confirms 12:15-14:15 and a GBP15 upper ticket price. Its zero lowPrice was not mistaken for general free entry; under-5 admission remains verify true. Existing Christmas Eve Five Go Jiving, priority-act lunches and suitable weekend rows were recognised despite URL/title variations.
  - Six existing Ewan Bleach/Fleeting Paradise lunches were individually reconciled with their artist ticket pages (WeGotTickets f/27249, 27252, 27255, 27258, 27261, 27265). Each confirms its specific date, all ages, doors 11:00 and music 12:30, with current ticket tiers. Jamboree's readable 4 October and 20 December pages still say 12:15 and older prices; four other venue pages failed. Updated the six rows to ticket URLs/prices and 12:30 with an explicit conflict note and verify true. Added a watchlist reminder; did not add duplicate gigs or silently treat the discrepancy as resolved.
- What did not work / cautions:
  - Spice of Life remains behind a verification challenge, so four lunchtime leads remain on the existing watchlist. Bookham/Winning Post ambiguous years, Epsom child-access gaps, distant Southend dates, U3A/member events, generic late central residencies and out-of-region tours did not clear the family threshold. The priority Doolally Sidcup evening is not a plausible toddler outing; appropriate nearby priority gigs were already held.
  - Reviewed the fresh 108-lead social export, especially posts since 22 September and all Lamb leads. The latest Lamb material is retrospective Mambo Jambo/Joao Mello or undetailed reels; other fresh posts were already-held TwickFolk shows, late/low-fit music or retrospective. No new social-only addition and no post timestamp used as an event date.
  - Council still advertises an October Celebrate Surbiton extension, but organiser copy still gives September Thursdays only. The expired September block was removed; the October lead stays unresolved. Christmas is a separately confirmed event, not an assumed extension.
- Source improvements: appended Riverhouse Barn's official event inventory and Deen City Farm's seasonal public programme, with structured-data and explicit-year/age cautions; added three focused searches. Now 91 known sources and 106 queries. Calendar source rows remain 53.
- Validation and boundary: ran npm run generate then npm run validate in a temporary repository-shaped copy because generation unconditionally writes JavaScript, ICS, icons, manifest and service worker outside the allowlist. Both passed; copied back only enriched calendar-data.json. Publishing must regenerate runtime assets. Corrected shell-pipe currency encoding in newly written fields before validation.
- Final counts: 313 events/feed events, 56 recurring patterns, 53 calendar sources, 89 watchlist entries; zero past-finished events, zero duplicate keys; latest end 25 April 2027 (range unchanged). Separately asserted unique IDs/row numbers, new rows above the original maximum, unchanged recurring records and unchanged retained event objects except the six documented Ewan updates. No commit or push.
- Try next time: resolve Jamboree/artist ticket start times; obtain Deen City Farm's explicit 2026 sessions and minimum ages; establish Wimbledon out-of-borough eligibility. Revisit Spice lunchtime detail pages, the October forecourt conflict and Fun Palaces' local listings only when more specific evidence is available. Continue crawling Surbiton's full all-category feed and checking alternate-title duplicates.
