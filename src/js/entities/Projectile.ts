/**
 * @fileoverview Projectile - Client-side projectile entity for visual representation
 * 
 * MIGRATION NOTES:
 * - Converted from Projectile.js following CLIENT_TYPESCRIPT_MIGRATION_PLAN Phase 3, Round 6
 * - Maintains 100% API compatibility with existing JavaScript callers
 * - Added comprehensive type definitions for projectile properties
 * - Preserved all visual effect and collision detection logic
 * 
 * ARCHITECTURE ROLE:
 * - Client-side visual representation of server-authoritative projectiles
 * - Handles smooth movement interpolation and visual effects
 * - Provides collision detection for client-side feedback
 * - Manages projectile lifecycle (creation, movement, deactivation)
 * 
 * SERVER-CLIENT PATTERN:
 * - Server ProjectileManager handles authoritative projectile logic
 * - Client Projectile entities provide visual feedback only
 * - Position updates smoothly interpolated for visual quality
 * - Collision detection runs on both client (feedback) and server (authority)
 * 
 * VISUAL SYSTEM:
 * - Base container with hitbox visualization (debug)
 * - Attachable animated sprite effects via ProjectileRenderer
 * - Rotation based on velocity direction for proper orientation
 * - Automatic cleanup when range exceeded or collision detected
 */
...
export class Projectile {
    ...
    private effectSprite: PIXI.AnimatedSprite | null;
    ...
    attachVisualEffect(effectSprite: PIXI.AnimatedSprite): void {
    ...
    checkCollision(entity: EntityInterface): boolean {
